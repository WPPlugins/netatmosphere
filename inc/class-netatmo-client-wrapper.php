...
class NetAtmo_Client_Wrapper {
	...
	*/
	...
	protected function __construct() {
        ...
        if( false && $this->isLocalhost() && file_exists( $this->netatmoLoginFilePath() ) ) {
            NAS_Plugin::debugFile('localhost mode');
            include( $this->netatmoLoginFilePath() );
			...
			try
			{
                ...
                $this->saveTokensToOptions($access_token, $refresh_token);
			}
			catch(Exception $ex)
			{
				NAS_Plugin::debugFile($ex);
			}
    ...
    protected static function saveTokensToOptions($access, $refresh) {
        ...
        NAS_Plugin::debugFile('tokens saved to options');
    }
    public function loadTokensFromOptions() {
        //NAS_Plugin::debugFile('loadTokensFromOptions');
        if( function_exists( 'get_option' ) ) {
            $access_token  = get_option( 'nas_netatmo_token_access' );
            $refresh_token = get_option( 'nas_netatmo_token_refresh' );
            
            //NAS_Plugin::debugFile($access_token);
            //NAS_Plugin::debugFile($refresh_token);
    ...
    public static function SaveTokensFromSession() {
        ...
        if( isset( $_SESSION['NETATMOSPHERE'] ) ) {
            ...
            if( isset( $_SESSION['NETATMOSPHERE']['ACCESS_TOKEN']) || isset( $_SESSION['NETATMOSPHERE']['REFRESH_TOKEN']) ) {
                NAS_Plugin::debugFile('tokens set');
